...
public class CommentAdapter extends RecyclerView.Adapter<CommentAdapter.CommentViewHolder> {
    ...
    public CommentAdapter(List<CommentHolder> commentHolderList, String postHolderId, String postId, String poster) {
    ...
    public void onBindViewHolder(@NonNull final CommentViewHolder holder, final int position) {
        ...
        // checks if it's the user's own question...
        if (currentUserId.equals(poster) || commentHolder.isRightAnswer()) {

            holder.image_make_right.setVisibility(View.VISIBLE);

        } else {

            holder.image_make_right.setVisibility(View.GONE);

        }

        // checks if it's the best answer by the one who asked the question...
        if (commentHolder.isRightAnswer()) {

            holder.image_make_right.setImageResource(R.drawable.right_answer_active);

        } else {

            holder.image_make_right.setImageResource(R.drawable.right_answer_disabled);
                        ...
                        replyHoldersCollection
                                ...
                                .addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
                                    ...
                                    public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                                        ...
                                        // if the current answer is chosen as the right answer...
                                        holder.image_make_right.setOnClickListener(new View.OnClickListener() {
                                            @Override
                                            public void onClick(View v) {

                                                if (!commentHolder.isRightAnswer()) {

                                                    makeRightAnswer(commentHolder, holder.image_make_right);

                                                } else {

                                                    makeWrongAnswer(commentHolder, holder.image_make_right);
    ...
    public class CommentViewHolder extends RecyclerView.ViewHolder {
        ...
        public CommentViewHolder(@NonNull View itemView) {
            ...
            image_make_right = itemView.findViewById(R.id.image_make_right);
    ...
    public void makeRightAnswer(final CommentHolder commentHolder, final ImageView imageView) {

        final ProgressDialog progressDialog = ProgressDialog.show(context, resources.getString(R.string.pleaseWait), resources.getString(R.string.makingRightAnswer), true);

        commentHoldersCollection
                .whereEqualTo("commentOnId", commentHolder.getCommentOnId())
                .get()
                .addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
                    @Override
                    public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                        ...
                        if (!queryDocumentSnapshots.isEmpty()) {
                            ...
                            for (QueryDocumentSnapshot queryDocumentSnapshot : queryDocumentSnapshots) {
                                ...
                                writeBatch.update(commentHoldersCollection.document(queryDocumentSnapshot.getId()), "rightAnswer", false);
                            }
                                        ...
                                        public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                                            ...
                                            if (!queryDocumentSnapshots.isEmpty()) {
                                                ...
                                                for (QueryDocumentSnapshot queryDocumentSnapshot : queryDocumentSnapshots) {
                                                    ...
                                                    writeBatch.update(commentsCollection.document(queryDocumentSnapshot.getId()), "rightAnswer", false);
                                                }

                                                writeBatch.update(commentHoldersCollection.document(commentHolder.getObjectId()), "rightAnswer", true);
                                                writeBatch.update(newsfeedCollection.document(postHolderId), "hasAnswer", true);

                                                writeBatch.update(commentsCollection.document(commentHolder.getCommentId()), "rightAnswer", true);
                                                writeBatch.update(postsCollection.document(postId), "hasAnswer", true);
    ...
    public void makeWrongAnswer(CommentHolder commentHolder, ImageView imageView) {

        final ProgressDialog progressDialog = ProgressDialog.show(context, resources.getString(R.string.pleaseWait), resources.getString(R.string.makingRightAnswer), true);

        WriteBatch writeBatch = db.batch();

        writeBatch.update(commentHoldersCollection.document(commentHolder.getObjectId()), "rightAnswer", false);
        writeBatch.update(newsfeedCollection.document(postHolderId), "hasAnswer", false);

        writeBatch.update(commentsCollection.document(commentHolder.getCommentId()), "rightAnswer", false);
        writeBatch.update(postsCollection.document(postId), "hasAnswer", false);
